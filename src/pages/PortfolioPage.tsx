import { Brain, ChevronRight, Grid2X2, LayoutDashboard, List, SlidersHorizontal } from "lucide-react";
    ...
    <>
      <section className="summary-grid">
        <SummaryCard title="Portfolio Value">
          ...
          <p className={totalPnl >= 0 ? "positive" : "negative"}>{totalPnl >= 0 ? "+" : ""}{money.format(totalPnl)} ({totalPnlPct.toFixed(2)}%)</p>
          <Sparkline data={[12, 16, 24, 19, 28, 32, 27, 20, 17, 22, 36, 42, 39, 31, 36, 47, 50]} tone="purple" />
        </SummaryCard>

        <SummaryCard title="Overall Risk ⓘ">
          <div className="score-line"><strong>{overallRisk}</strong><span>/100</span></div>
          ...
          <div className="risk-meter"><span style={{ width: `${overallRisk}%` }} /></div>
          <div className="meter-labels"><span>0</span><span>100</span></div>
        </SummaryCard>

        <SummaryCard title="Daily PNL">
          <h2 className={totalPnl >= 0 ? "positive" : "negative"}>{totalPnl >= 0 ? "+" : ""}{money.format(totalPnl)}</h2>
        ...
        <SummaryCard title="Positions">
          <div className="positions-summary">
            <div><h2>{positions.length}</h2><p>Diversified</p></div>
      ...
      <section className="insight-card">
        <div className="insight-copy">
          <Brain size={34} />
          <div><h2>Portfolio Insight</h2><p>{portfolioInsight}</p></div>
        </div>
        <div className="insight-visual">
          <Sparkline data={[20, 28, 24, 31, 33, 29, 36, 43, 38, 50, 62, 58, 54, 59, 66, 61, 57, 69, 72, 63, 60, 73]} tone="purple" />
        </div>
        <button type="button" className="primary-button" onClick={() => setActiveView("insights")}>View Details <ChevronRight size={18} /></button>
      </section>
      ...
      <div className="content-grid">
        <section className="positions-panel">
          ...
          {portfolioViewMode === "list" && <div className="table-head"><span>Position</span><span>Value / PNL</span><span>Risk Score</span><span>Action</span></div>}

          <div className={portfolioViewMode === "grid" ? "position-grid" : "position-list"}>
            {sortedPositions.map((position) =>
              portfolioViewMode === "grid" ? (
                <PositionCard key={position.ticker} position={position} onOpen={() => setExpandedTicker(position.ticker)} />
              ) : (
                <PositionRow key={position.ticker} position={position} isExpanded={expandedTicker === position.ticker} onToggle={() => setExpandedTicker(expandedTicker === position.ticker ? null : position.ticker)} />
              )
            )}
          </div>
        </section>

        <PortfolioRightRail riskDistribution={riskDistribution} topDrivers={topDrivers} setActiveView={setActiveView} />
      </div>
    </>
  );
}

function PortfolioRightRail({ riskDistribution, topDrivers, setActiveView }: { riskDistribution: { value: number; level: RiskLevel }[]; topDrivers: DriverWithTicker[]; setActiveView: (view: ViewType) => void }) {
  return (
